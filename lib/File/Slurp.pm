...
my @edit_export   = qw(edit_file edit_file_lines);
my @abbrev_export = qw(rf wf ef efl);

our @EXPORT_OK = (@edit_export, @abbrev_export, qw(slurp prepend_file));
our %EXPORT_TAGS = (
    'all'  => [@EXPORT, @edit_export, @abbrev_export, @EXPORT_OK],
    'edit' => [@edit_export],
    'std'  => [@EXPORT],
    'abr'  => [@abbrev_export],
);
...
sub read_file {
        ...
        && !wantarray)
    {
        ...
        unless (defined $read_cnt) {
            @_ = ($opts, "read_file '$file_name' - small sysread: $!");
            goto &_error;
        }
    ...
    if (ref $file_name) {
        ...
        if ($ref_result) {
            ...
            # we got an overloaded object and the result is the stringified
            # value use it as the file name
            $file_name = $ref_result;
        }
        else {
            # here we have just an open handle. set $read_fh so we don't do
            # a sysopen
            $read_fh   = $file_name;
    ...
    unless ($read_fh) {
        ...
        # we need a blk_size if the size is 0 so we can handle pseudofiles like
        # in /proc. these show as 0 size but have data to be slurped.
        unless ($size_left) {
            $blk_size = $opts->{'blk_size'} || 1024 * 1024;
            $size_left = $blk_size;
        }
    ...
    while (1) {
        ...
        my $read_cnt
            = sysread($read_fh, ${$buf_ref}, $size_left, length ${$buf_ref});
...
sub write_file {
    ...
    unless ($write_fh) {
        ...
        if ($opts->{'atomic'}) {
            ...
            # in atomic mode, we spew to a temp file so make one and save the
            # original file name.
    ...
    # get the size of what we are writing and init the offset into that buffer
    ...
    do {
        ...
        my $write_cnt = syswrite($write_fh, ${$buf_ref}, $size_left, $offset);
    ...
    # file so seek to the current position to get it (same as tell()).
    truncate($write_fh, sysseek($write_fh, 0, SEEK_CUR)) unless $no_truncate;
    close($write_fh);
...
sub prepend_file {
    ...
    my @bad_opts = grep $_ ne 'err_mode' && $_ ne 'binmode', keys %{$opts};
    ...
    $opts->{atomic} = 1;
    my $write_result
        = eval { write_file($file_name, $opts, $prepend_data, $$existing_data); };
...
sub edit_file(&$;$) {
    ...
    $opts->{atomic} = 1;
    my $write_result = eval { write_file($file_name, $opts, $edited_data) };
...
my %err_func = ('carp' => \&carp, 'croak' => \&croak,);
